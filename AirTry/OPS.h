...
public:
	...
	void print_kml() {
		...
		if (asp->drop == 1)
		{
			...
			geo_drop_point = model->TSK_to_Geo(X_la, 0) * 180 / M_PI;
			Point_ASP.push_back(geo_drop_point);

			d = model->norm2svyaz(X_la, 0, 0, model->X[4]);
			d[0] += A;
			d = model->svyaz_to_norm(d, 0, 0, model->X[4]);
			geo_land_point = model->TSK_to_Geo(d, 0) * 180 / M_PI;
	...
	{
		if (abs(x_range - A) < 120 && asp->drop == 0)
		{
			asp->drop = 1;
		}
	}
